...
Use the exact `dataset_manifest.csv` v2 columns. Run `dataset_tool.py validate <manifest> <dataset-root> --require-reviewed` before freezing an evaluation set and `dataset_tool.py split` before training. The validator checks hashes, exact duplicates, paths, review state and production-lot/session leakage. Optional perceptual hashing only proposes near-duplicate pairs; a domain reviewer must disposition them.

Ambiguous, occluded or visually unresolvable samples are `unknown`/HOLD candidates. Reviewers must not force them into PASS or FAIL to improve metrics. Original annotations and both independent reviews remain immutable; adjudication produces a new version linked to the originals.
