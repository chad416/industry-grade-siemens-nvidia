...
def _inputs_required() -> list[dict]:
    items = [
        ...
        ("IR-019","FreeCAD installation and qualified panel-CAD reviewer","Electrical owner","Revision-D.1 FCStd/exchange validation"),
        ...
        ("IR-021","Usable QElectroTech installation and qualified electrical schematic reviewer","Electrical owner","Revision-D.1 schematic authoring, reopen, export and review"),
...
def _gates() -> list[dict]:
    gates = [
        ...
        (2,"Canonical model reconciles schedules","PASS","Revision-D.1 deterministic validator and source-parity checks pass; see current automated report for count"),
        ...
        (10,"Revision-D.1 QET reopens and reconciles","BLOCKED","No runnable QElectroTech found; historical revision-A native baseline only"),
        (11,"Schematics visual review","BLOCKED","Revision-D.1 native schematics do not exist"),
        (12,"Revision-D.1 FCStd reopens","BLOCKED","No runnable FreeCAD found; historical baseline only"),
        (13,"STEP/IGES/DXF reimport","BLOCKED","Revision-D.1 exchange files do not exist and FreeCAD is absent"),
        ...
        (19,"PLC-NVIDIA failure tests","PASS","Local source-design scope: simulator, 32 scenarios, edge service, interface harness and Revision-D.1 independent interface-oracle tests pass; native integration remains blocked under gates 9 and 18"),
...
def _write_control(root: Path, model: dict) -> None:
    _csv(root,"00_project_control/revision_history.csv",[
        ...
        {"revision":"D.1","date":"2026-08-03","status":"Release candidate","description":"Git-object release integrity, retained PLC transaction identity, post-fault disposition, immutable edge publication and clean-clone reproduction"},
    ])
    _csv(root,"00_project_control/engineering_change_register.csv",[
        ...
        {"ecr":"ECR-D1-001","reason":"Make committed Git objects the release byte authority","affected":".gitattributes; QET/DXF blobs; manifest/determinism/reproduction scripts; CI","compatibility":"Text is LF; native/exchange/binary artifacts are byte-exact; QET/DXF CRLF originals are force-readded","test_impact":"Fresh-clone validator, staged/head manifest verification, superseded-path denial and two-snapshot determinism","risk":"Self-hosted artifact CI requires the controlled workbook/PDF runtime","migration":"Build manifests only from a complete staged index and accept releases only after clean-clone HEAD reproduction"},
        {"ecr":"ECR-D1-002","reason":"Harden PLC restart identity, startup decommand, capper correlation and post-fault disposition","affected":"revision_d_scl.py; generated Siemens SCL; D.1 Siemens contract tests","compatibility":"Session/model configuration is retained; restart with captured product requires removal and explicit disposition","test_impact":"Ten new static Siemens source-contract regressions plus full simulator suite","risk":"Native TIA compile, retentivity configuration and PLCSIM behavior remain unverified","migration":"Import all 15 sources together; configure approved model identity before enabling vision"},
        {"ecr":"ECR-D1-003","reason":"Reject same-session replay and preserve immutable edge results until exact acknowledgement","affected":"edge protocol/service/tests","compatibility":"Canonical ASCII model IDs; coherent disabled reset cannot clear an unacknowledged same-session result","test_impact":"Seven additional replay, polling, exception-containment and publication-lifecycle tests","risk":"No production OPC UA adapter or target Jetson runtime exists","migration":"Adapter must pass a coherent disabled PLC snapshot and retain exact ACK semantics"},
    ])
    ...
    _csv(root,"00_project_control/standards_sources_register.csv",[
        ...
        {"source":"NVIDIA DeepStream 9.1 release notes","url":"https://docs.nvidia.com/metropolis/deepstream/9.1/text/DS_Release_notes.html","accessed":"2026-08-03","applied_to":"Jetson Orin target runtime planning: JetPack 7.2 GA / L4T 39.2 / TensorRT 10.16.1.7 baseline","claim_limit":"DeepStream, JetPack and target Jetson were not installed or executed"},
        {"source":"NVIDIA TensorRT documentation","url":"https://docs.nvidia.com/deeplearning/tensorrt/latest/","accessed":"2026-08-03","applied_to":"Generic engine/runtime deployment planning only","claim_limit":"Generic TensorRT 11.1 documentation is not the DeepStream 9.1 Jetson compatibility baseline; TensorRT is not installed and no engine exists"},
    ])
    _csv(root,"00_project_control/decision_register.csv",[
        ...
        {"decision_id":"ADR-D1-001","decision":"Git staged/index and HEAD blobs are the only release-byte authorities","rationale":"Revision D manifest hashes were created from OneDrive CRLF worktree bytes that differed from published Git blobs","consequence":"Manifest build rejects unstaged/untracked inputs; final acceptance requires clean-clone HEAD verification"},
        {"decision_id":"ADR-D1-002","decision":"PLC session identity and approved model configuration are retained while all commands are explicitly decommanded on startup","rationale":"A restart must not reuse the prior transaction namespace or re-energize stale requests","consequence":"Native TIA retentivity and OB100 behavior require compile/PLCSIM confirmation"},
        {"decision_id":"ADR-D1-003","decision":"An edge result remains immutable until exact same-session acknowledgement","rationale":"Transport or heartbeat faults must not silently erase the publication needed for deterministic PLC cleanup","consequence":"Only a serially advanced disabled session may invalidate prior-session publication, and that event is recorded"},
    ])
    ...
    _csv(root,"14_qa/review_records.csv",[
        ...
        {"review_record_id":"RR-D1-001","reviewer_task":"/root/d1_release_integrity","review_type":"Independent software-agent defect reproduction","scope":"Published Revision-D Git objects, manifest, clean-clone determinism and OneDrive contamination","method":"Fresh GitHub clone at f1e34bd; Git blob/worktree byte analysis; manifest and reproduction execution; parent-history comparison of twelve resurrected paths","evidence_sha256":digest([".gitattributes","scripts/build_manifest.py","scripts/verify_manifest.py","scripts/check_determinism.py","scripts/test_release_integrity.py","14_qa/release_integrity_reproduction.md"]),"limitations":"Final D.1 commit and GitHub clone did not exist during this audit; no native engineering-tool review","actual_qualification":"Software agent; not an identified qualified human engineer","human_approval_required":"Yes","disposition":"FINDINGS CONFIRMED AND CORRECTED - QET/DXF Git-byte defect, working-tree manifest authority, stale-path ingress and runtime pinning were addressed; final commit and clean-clone evidence remain mandatory"},
        {"review_record_id":"RR-D1-002","reviewer_task":"/root/d1_siemens_readiness","review_type":"Implementation workstream software-agent record","scope":"Revision-D.1 Siemens restart identity, startup decommand, capper correlation, HMI arbitration, diagnostics and post-fault product disposition","method":"Bounded SCL-generator/source implementation plus ten new source-contract tests and full simulator regression","evidence_sha256":digest(["scripts/revision_d_scl.py","04_controls_siemens/scl/DB_Global.scl","04_controls_siemens/scl/FB_CellMain.scl","04_controls_siemens/scl/OB100_Startup.scl","11_simulation/tests/test_revision_d1_siemens_contracts.py"]),"limitations":"Authoring workstream, not independent approval; no TIA/WinCC compile, Startdrive, PLCSIM, hardware or qualified controls review","actual_qualification":"Software agent; not an identified qualified human engineer","human_approval_required":"Yes","disposition":"IMPLEMENTED AND LOCALLY TESTED - 10/10 added contracts and 58/58 full simulator tests; all native and human gates remain blocked"},
        {"review_record_id":"RR-D1-003","reviewer_task":"/root/d1_nvidia_readiness","review_type":"Implementation workstream software-agent record","scope":"Immutable result publication, session reset/regression rejection, exact acknowledgement and canonical model identity","method":"Bounded protocol/service implementation with edge-service and PLC-interface regression tests","evidence_sha256":digest(["07_nvidia_vision/edge_service/protocol.py","07_nvidia_vision/edge_service/service.py","07_nvidia_vision/edge_service/tests/test_service.py","07_nvidia_vision/test_plc_interface_harness.py"]),"limitations":"Authoring workstream, not independent approval; no real OPC UA endpoint, dataset, model, DeepStream/TensorRT/Jetson deployment or qualified cybersecurity review","actual_qualification":"Software agent; not an identified qualified human engineer","human_approval_required":"Yes","disposition":"IMPLEMENTED AND LOCALLY TESTED - 42/42 edge and 15/15 interface-harness tests; target runtime, model and physical integration remain blocked"},
        {"review_record_id":"RR-D1-004","reviewer_task":"/root/rev_d_electrical_audit","review_type":"Independent software-agent Revision-D.1 audit","scope":"Electrical schedules, historical QET/FCStd/exchange files, panel boundary and executable availability","method":"Read-only schedule reconciliation, native-structure parsing, executable/registry inventory and visual inspection of all 24 historical schematic pages","evidence_sha256":digest(["03_electrical/native_baseline/filling_cell.qet","03_electrical/native_baseline/filling_cell_schematics.pdf","09_panel_cad/native_baseline/mounting_plate.dxf","10_schedules/point_to_point_connections.csv","10_schedules/panel_placement.csv"]),"limitations":"No QElectroTech/FreeCAD executable, D.1 native authoring/reopen/reimport, confirmed site inputs, construction check or qualified electrical approval","actual_qualification":"Software agent; not an identified qualified human engineer","human_approval_required":"Yes","disposition":"FINDINGS CONTROLLED - schedules reconcile internally and historical files are byte-exact but obsolete; conductor/protection/PE/thermal/site inputs and all D.1 native gates remain open"},
        {"review_record_id":"RR-D1-005","reviewer_task":"/root/independent_vision_review","review_type":"Final independent integrated software-agent audit","scope":"Frozen D.1 release integrity, Siemens/edge contracts, schedules, safety wording, test evidence, workbook, PDF and blocker truthfulness","method":"Read-only adversarial source review; 58/42/15 tests; 32-scenario external regeneration; toolchain verification; isolated dry-stage manifest; workbook/PDF visual inspection; 428-check follow-up of source-neutral index/HEAD determinism evidence","evidence_sha256":digest([".gitattributes","scripts/revision_d_scl.py","07_nvidia_vision/edge_service/service.py","00_project_control/canonical_model.json","10_schedules/FC01_engineering_schedules.xlsx","release/FC01_release_evidence.pdf","release/reproduction_toolchain_lock.json"]),"limitations":"No final manifest freeze or committed clean-clone evidence at audit time; no native TIA/WinCC/Startdrive/PLCSIM, D.1 QET/FreeCAD, site-dependent electrical/conductor inputs, real dataset/model, ONNX/TensorRT/DeepStream/Jetson/OPC UA integration, hardware, FAT/SAT, cybersecurity acceptance, qualified safety or qualified-human approval","actual_qualification":"Software agent; not an identified qualified human engineer","human_approval_required":"Yes","disposition":"ACCEPT FOR CONTROLLED MANIFEST FREEZE - no remaining locally closable engineering/source-design blocker; final manifest freeze and committed fresh-clone verification remain mandatory before release; push remains pending"},
    ])
    rationale_lines = ["# Component rationale register - Revision D.1","",f"> {NOTICE}","",f"> {SAFETY}","","Generated from the controlled canonical model. Software-agent consistency review is not qualified-human approval.","","| Category | Key | Purpose | Failure detected or controlled | Basis | Open verification | Owner |","|---|---|---|---|---|---|---|"]
...
def apply_revision_d(root: Path) -> None:
    ...
    trace = {
        ...
        "SYS-023":("00_project_control/canonical_model.json; scripts/validate_project.py","VAL-D-STATIC","Revision-D.1 canonical/source/schedule validation suite; the executed report records the current check count"),
    ...
    model["project"] = {"id":"FC01","revision":"D.1","date":"2026-08-03","notice":NOTICE,"safety_boundary":SAFETY,"status":"PROFESSIONAL CONTROLLED ENGINEERING-DEVELOPMENT RELEASE CANDIDATE; CONSTRUCTION AND DEPLOYMENT RELEASE PENDING EXPLICIT NATIVE, PHYSICAL AND QUALIFIED ACCEPTANCE GATES"}
...
Maturity: **Professional controlled engineering-development package; construction and deployment release pending the explicitly listed native, physical and qualified acceptance gates.**"""))
    _write(root,"release/RELEASE_NOTES.md",_doc("Revision D.1 release notes", """This controlled engineering-development release candidate includes generator-owned Revision-D.1 SCL, schedules, independent models, edge contract, documentation, workbook/PDF and deterministic manifest evidence. Manifest rows are built from staged Git-index bytes and final integrity is verified against committed HEAD bytes from a clean clone.
...
Native Siemens compilation, HMI project, Startdrive, PLCSIM, revised QET/FreeCAD, confirmed site-dependent electrical calculations, trained NVIDIA model/runtime and physical acceptance remain blocked. No construction, deployability, compliance, safety-performance, FAT, SAT or AI-performance claim is made."""))
    _write(root,"00_project_control/repository_release_workflow.md",_doc("Repository and release-byte workflow - Revision D.1", """## Authority and location
...
Never repair a release by editing manifest hashes. Reproduce the mismatch against Git blobs, establish the intended source bytes, change the bytes or policy under an ECR, rebuild from staged index bytes, and repeat clean-clone verification."""))
    _write(root,"14_qa/release_integrity_reproduction.md",_doc("Published Revision-D release-integrity reproduction", """A fresh GitHub clone of commit `f1e34bd15b29ecd2fc6a8f0110861a721ee5b9d1` produced `PASS=396 FAIL=1`; the failed check was the historical QElectroTech baseline hash. The pristine manifest reported 243 listed and 243 actual controlled files with four discrepancies: QET size/hash and DXF size/hash.
...
The contaminated OneDrive checkout contained exactly twelve untracked Revision-C files. Every one matched its recoverable Revision-C Git blob. D.1 denies their exact paths and its preflight runs before any generator removal, preventing cleanup from hiding contamination."""))
    _write(root,"00_project_control/final_release_checklist.md",_doc("Final release checklist - Revision D.1", """- [x] Revision-D.1 canonical schedules regenerate from one owner.
...
- [ ] Revision-D.1 QET metadata, continuity, BOM and cross-references pass.
- [ ] Revision-D.1 FCStd and exchange formats reopen with selected device envelopes.
...
Unchecked items are release blockers for construction or deployment, not missing claims."""))
  ...
  R[Approved requirements and site inputs] --> C[Revision-D.1 canonical model]
  C --> S[Controlled schedules]
  C --> PLC[Siemens source design]
  C --> HMI[HMI tag/command contract]
  C --> V[Vision protocol and simulator]
  S --> QET[Revision-D.1 QET - blocked]
  S --> CAD[Revision-D.1 panel CAD - blocked]
...
- No SCCR, short-circuit withstand, selectivity, discrimination or final thermal-compliance claim is made."""))
    _write(root,"14_qa/executed_test_report.md",_doc("Executed non-native test report - Revision D.1", """Execution date: 2026-08-03. Runtime: bundled Python 3.12. These are deterministic source/design/interface tests, not TIA Portal compile, PLCSIM, FAT, SAT or physical commissioning evidence.
...
`python scripts/validate_project.py` passes the current static/data/source check set with zero failures. `scripts/check_determinism.py` independently rebuilds the controlled source/report set twice with zero missing, extra or mismatched files; the release entry point additionally compares two normalized workbook builds and two PDF builds by SHA-256. The workbook renders 20 sheets with zero formula-error matches; the PDF renders five pages. Final manifest verification must report equal listed/actual controlled-file counts and zero missing, unlisted, classification or hash mismatches. Native/tool-dependent blockers remain open regardless of these passes."""))
    _write(root,"14_qa/visual_review_report.md",_doc("Visual artifact review report - Revision D.1", f"""Review date: 2026-08-03. Renderers: bundled `@oai/artifact-tool` for XLSX and Poppler for PDF rasterization.
...
`03_electrical/native_baseline/filling_cell_schematics.pdf` remains a 24-page QElectroTech-generated Revision-A baseline, and the inherited FCStd/exchange set remains Revision-A evidence. Historical renders are legible but do not contain the selected Revision-D.1 Siemens/NVIDIA design. They are quarantined baselines only, not accepted Revision-D.1 electrical or panel-CAD deliverables."""))
    _write(root,"14_qa/acceptance_gate_status.md",_doc("Acceptance gate status — Revision D.1", "\n".join([f"- Gate {g['gate']}: **{g['status']}** — {g['acceptance_gate']}: {g['evidence_or_blocker']}" for g in model["acceptance_gates"]])))
    _write(root,"00_project_control/design_basis.md",_doc("Design basis - Revision D.1", """The only current canonical owner is `00_project_control/canonical_model.json`, finalized by `scripts/revision_d_generator.py`; `scripts/revision_d_scl.py` owns all 15 Siemens source exports. Schedules, rationale, documents, workbook, PDF, validator evidence and manifests derive downstream in the recorded dependency order.
...
The PLC remains authoritative for sequence, interlocks, timeouts, safe decommanding, recovery and transfer permission. NVIDIA is a non-safety quality subsystem. Historical QET/FreeCAD files are quarantined Revision-A evidence and never represent selected Revision-D.1 hardware. Site supply, fault current, motor/pump data, cable routes, environmental limits, qualified safety work, native engineering, real dataset/model/runtime and physical testing remain controlled inputs/gates."""))
    _write(root,"02_system_architecture/interface_control_document.md",_doc("PLC-NVIDIA interface control document - Revision D.1", """## Atomic transaction
...
- S615 configuration manual: https://support.industry.siemens.com/cs/attachments/109976097/PH_SCALANCE-S615-WBM_76.pdf"""))
    _write(root,"09_panel_cad/README.md",_doc("Panel CAD boundary - Revision D.1", """The placement schedule contains collision-checked reservation coordinates for known envelopes and separate door-HMI placement, but unresolved drive/firewall/Jetson dimensions, complete BOM placement, thermal losses, duct fill and manufacturer clearances prevent release. The inherited FCStd/STEP/IGES/DXF files remain byte-exact Revision-A evidence. No runnable FreeCAD was found and no Revision-D.1 reopen, collision study or reimport is claimed."""))
    _write(root,"14_qa/source_audit_report.md",_doc("Source audit report - Revision D.1", """Generator-level review confirmed and corrected the Revision-D release-byte authority defect, PLC restart/session replay boundary, post-fault product disposition, capper pre-busy correlation, HMI local/remote ledger isolation, startup decommand coverage, edge same-session replay and immutable-publication lifecycle in addition to the Revision-D vision/fill findings.
...
Historical QET/FreeCAD artifacts remain genuine but non-current. Static/source and independent Python checks do not establish Siemens compilation, PLCSIM equivalence, electrical construction, functional safety, AI performance or physical behavior."""))
    _write(root,"14_qa/final_gate_review.md",_doc("Final locally achievable gate review - Revision D.1", """Locally executable generation, direct interface oracles, simulator scenarios, edge-service/harness tests, schedule/source validation, workbook/PDF rendering, staged-index manifest construction and clean-clone HEAD verification are required to pass in the final reproduction record.
