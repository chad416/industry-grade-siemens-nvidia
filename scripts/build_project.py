...
hardware = [
    ...
    {"tag":"-SW100","role":"Industrial Ethernet switch","manufacturer":"Siemens","model":"SCALANCE XB208 unmanaged","order_no":"6GK5008-0BA10-1AB2","qty":1,"status":"Preliminary; official compatibility/procurement check open","basis":"Eight ports allow PLC, HMI, two drives, vision and service with spare ports"},
]
...
write("03_electrical/README.md",doc("Electrical workstream status", """`native_baseline/filling_cell.qet` and its PDF are genuine inherited artifacts from the stronger primary package. They were previously reopened using QElectroTech 0.100.1-dev (24 diagrams, 305 elements, 160 conductors), but they predate the final Siemens/NVIDIA selection. They are quarantined as baseline evidence and are **not** represented as the completed revision-B multiline upgrade.
...
write("04_controls_siemens/scl/OB1_Call_Structure.scl",f'''// {NOTICE}\n// REVIEWABLE CALL STRUCTURE ONLY. Hardware IDs, instance DB names and actual tag bindings must be created and compiled in TIA.\nORGANIZATION_BLOCK "Main"\nVAR_TEMP FirstScan : Bool; END_VAR\nBEGIN\n+  // 1) Read/normalize physical I/O and technology-object counters.\n+  // 2) Call FB_VFD instances for conveyor and pump.\n+  // 3) Call FB_Actuator2Pos instances for gate and clamp.\n+  // 4) Call two FB_FillChannel instances with independent pulse totals.\n+  // 5) Call FB_VisionInterface, FB_RecipeManager and capping handshake module.\n+  // 6) Call FB_MachineCoordinator and FB_AlarmManager.\n+  // 7) Map commands to outputs through final permissive/interlock layer; release build forces all simulation hooks FALSE.\n+END_ORGANIZATION_BLOCK''')
