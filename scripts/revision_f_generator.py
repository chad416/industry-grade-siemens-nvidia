...
def apply_revision_f(root: Path) -> None:
    ...
    for row in [
        ...
        {"ecr":"ECR-F-006","reason":"Contain platform-level workbook raster nondeterminism without concealing engineering-content changes","affected":"scripts/build_workbook.mjs; scripts/workbook_render_stability.mjs; scripts/test_workbook_render_stability.mjs; 14_qa/workbook_render_baseline.json; reproduction workflow","compatibility":"Reviewed Git PNGs remain authoritative evidence while normalized XLSX and CSV authorities still rebuild exactly","test_impact":"Sixteen fail-closed PNG stability tests plus complete clean-clone generation and final Git-clean assertion","risk":"An overly broad tolerance could mask a visual defect; tolerance is therefore bound to approved input/builder hashes and limited to 16 pixels, 64-pixel bounding-box area and 5e-6 image fraction","migration":"Render candidates outside the release tree; require explicit FC01_UPDATE_WORKBOOK_RENDERS=1 only after visual review for baseline changes; prohibit update mode during reproduction"},
    ]:
    ...
    for row in [
        ...
        {"decision_id":"ADR-F-005","decision":"Treat workbook PNGs as reviewed evidence with a fail-closed semantic-hash and bounded raster-equivalence policy","rationale":"The workbook and source schedules rebuilt byte-identically while the portable renderer changed six isolated pixels in one 4.17-million-pixel sheet between clean clones","consequence":"Tolerance requires approved authoritative-input and builder hashes; missing, unexpected, non-regular, dimension-changed or material drift fails; intentional updates require an explicit review flag and are prohibited in reproduction"},
    ]:
...
- [ ] Native Siemens, real dataset/model/runtime, site electrical, physical and qualified-safety gates are closed externally."""))
    ...
    if "## Controlled workbook raster evidence" not in release_workflow:
