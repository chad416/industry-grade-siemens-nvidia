# Determinism report - Revision D.1

Result: **PASS**

Two independent temporary exports of authoritative Git index bytes ran canonical generation, all 32 scenarios and read-only deterministic validation.
The comparison covers controlled text, CSV, JSON, Siemens SCL, scripts and generated reports. XLSX and PDF binary determinism is checked separately by the standard reproduction workflow.

Compared files: 182
Missing from run B: 0
Extra in run B: 0
Hash mismatches: 0
